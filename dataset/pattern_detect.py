...
# 数据引入
excel_file_path = \
        r"D:\科研\可控场景生成\Diffusion model\diffusion model for controllable scenario generation\dataset\WDdata.xlsx"
weekly_data = excel8760_to_daily24(
...
for i in range(weekly_data.shape[0]):
    ...
    # 添加首尾差值
    circular_diff = np.abs(vector[-1] - vector[0])
    total_diff = sum(diffs) + circular_diff
    if data_set == "pv":
        pattern = [max_val, mean_val, non_zero_ratio, total_diff]
    elif data_set == "wd":
        pattern = [max_val, mean_val, min_val, total_diff]
    else:
...
max_diffs_global = np.max([r[3] for r in total_patterns])
min_diffs_global = np.min([r[3] for r in total_patterns])
pattern_max = [max_values_global, max_mean_values_global, max_non_zero_ratios_global, max_diffs_global]
pattern_min = [min_max_values_global, min_mean_values_global, min_non_zero_ratios_global, min_diffs_global]
print("总体统计信息:")
...
print(f"波动值范围：{max_diffs_global:.4f}~{min_diffs_global:.4f}")
