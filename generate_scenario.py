...
class ModelLoader:
    ...
    def load_model(self):
        ...
        self.pattern_features_max = checkpoint.get("pattern_features_max", np.ones(7))
    ...
    for i, sample in enumerate(y_gen_real):
       ...
       # 根据数据集类型设置标题
        if model_loader.dataset_type == "PVDataSet":
            ...
            plt.title(f'Generated Weekly PV Profile {i + 1}\n{feature_info}')
        else:
            ...
            plt.title(f'Generated Weekly WD Profile {i + 1}\n{feature_info}')
        plt.tight_layout()
...
def main():
    ...
    try:
        ...
        if model_loader.dataset_type == "PVDataSet":
            ...
            x_target = np.array([[0.1323, 0.551, 89.0, 0.694, 0.217, 0.0368, 0.0019]])  # 示例特征
        else:
